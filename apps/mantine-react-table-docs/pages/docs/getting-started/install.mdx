...
> This is the install guide for `mantine-react-table` **V2** and `@mantine/` packages **V8**. MRT V2 requires Mantine V8.3.9 or later.
...
> `mantine-react-table` V2 requires **Mantine V8.3.9+** packages as dependencies in your project.
>
...
>
> 1.  `@mantine/core` (^v8.3.9+)
> 2.  `@mantine/hooks` (^v8.3.9+)
> 3.  `@mantine/dates` (^v8.3.9+)
...
The Quick Install instructions here assume you already have Mantine V8.3.9+ installed and correctly configured with postcss in your project.

<InstallCommand packagesString="mantine-react-table@beta" />

### Install With Required Peer Dependencies (Recommended)

It is best to refer to the [Mantine installation guide](https://mantine.dev/getting-started/) for the most up-to-date instructions on how to install Mantine V8 for your specific React meta framework. (Next.js, Remix, Vite, etc.)
...
The [Mantine installation guide](https://mantine.dev/getting-started/) has much better instructions for how to set up your Mantine theme, but you should have something like this in your project:
...
If you don't see any styles applied to the mantine table components, you may have mismatching versions of `mantine-react-table` and `@mantine/core`. MRT v2 requires Mantine v8.3.9 or later.
...
Make sure all mantine packages are at least v8.3.9.
  ...
  faqStructuredData={{
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: 'Yes. MRT v2 requires Mantine v8.3.9 or later.',
        },
      },
      {
        '@type': 'Question',
